        ...
        <div className="fpv__layout">
          ...
          <main className="fpv__main">
            ...
            <div className="fpv__list">
              ...
              {projects.length === 0 && (
                <EmptyState text="No projects match your filters." />
              )}
            </div>
          </main>
        </div>
...
function TopRow({ count }) {
  return (
    <div className="fpvTopRow">
      <div className="fpvTopRow__count">{count} projects found</div>
...
function FiltersPanel({ filters }) {
  return (
    <section className="fpvFilters">
      ...
      <FilterSection title={`Minimum Match Score: ${filters.minMatch}%`}>
        <input
          className="fpvSlider"
          type="range"
          min="0"
      ...
      {options.map((op) => {
        ...
        return (
          <button
            key={op.value}
            type="button"
            className={`fpvPills__pill ${isActive ? "is-active" : ""}`}
...
function ProjectCard({ project, onOpen, onApply }) {
  return (
    <article className="fpvCard fpvCard--clickable" onClick={onOpen} role="button" tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      ...
      <div className="fpvCard__bottom">
        ...
        <button
          ...
          type="button"
          onClick={(e) => {
            e.stopPropagation(); // يمنع فتح الكارد مرتين
            onApply();
          }}
        >
          Apply Now
